...
/* ! Medoo 0.8.2 - Copyright 2013, Angel Lai - MIT license - http://medoo.in */
...
class medoo {
            ...
            if (($key == 'AND' || $key == 'OR') && is_array($value)) {
                ...
                if (isset($match[3])) {
                    ...
                    if (is_int($key)) {
                        ...
                        $wheres[] = is_array($value) ? $match[1] . ' IN (' . $this->array_quote($value) . ')' : $match[1] . ' = ' . $this->quote($value);
                    }
                }
            }
    ...
    public function select($table, $columns, $where = null) {
        $query = $this->query('SELECT ' . (is_array($columns) ? implode(', ', $columns) : $columns) . ' FROM ' . $table . $this->where_clause($where));
